...
namespace ngk { namespace chain {
   ...
   void protocol_feature_activation::reflector_init() {
      ...
      NGK_ASSERT( protocol_features.size() > 0, ill_formed_protocol_feature_activation,
      ...
      for( const auto& d : protocol_features ) {
         auto res = s.insert( d );
         NGK_ASSERT( res.second, ill_formed_protocol_feature_activation,
   ...
   )
   {
      ...
      NGK_ASSERT( !enforce_disjoint || protocol_features.size() == expected_size,
