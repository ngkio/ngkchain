...
namespace ngk { namespace chain {
    ...
    *  In the context of ngk, every action is dispatched to the handler defined
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         NGK_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         NGK_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT( ngk::chain::permission_level, (actor)(permission) )
FC_REFLECT( ngk::chain::action, (account)(name)(authorization)(data) )
