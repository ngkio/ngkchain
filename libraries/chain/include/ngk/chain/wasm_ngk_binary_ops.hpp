...
namespace ngk { namespace chain { namespace wasm_ops {
...
struct field_specific_params {
   ...
   static void pack(instruction_stream* stream, Field& f) { return ngk::chain::wasm_ops::pack(stream, f); }
   static auto to_string(Field& f) { return std::string(" ")+
                                       ngk::chain::wasm_ops::to_string(f); }
};
template <>
...
template <class Op_Types>
struct NGK_OperatorDecoderStream
{
   NGK_OperatorDecoderStream(const std::vector<U8>& codeBytes)
   ...
   instr* decodeOp() {
      NGK_ASSERT(nextByte + sizeof(IR::Opcode) <= end, wasm_exception, "");
      ...
      #define VISIT_OPCODE(opcode,name,nameString,Imm,...) \
         case IR::Opcode::name: \
         { \
            NGK_ASSERT(nextByte + sizeof(IR::OpcodeAndImm<IR::Imm>) <= end, wasm_exception, ""); \
...
template <class Op_Types>
const std::vector<instr*>* NGK_OperatorDecoderStream<Op_Types>::_cached_ops;

}}} // namespace ngk, chain, wasm_ops

FC_REFLECT_TEMPLATE( (typename T), ngk::chain::wasm_ops::block< T >, (code)(rt) )
FC_REFLECT( ngk::chain::wasm_ops::memarg, (a)(o) )
FC_REFLECT( ngk::chain::wasm_ops::blocktype, (result) )
FC_REFLECT( ngk::chain::wasm_ops::memoryoptype, (end) )
