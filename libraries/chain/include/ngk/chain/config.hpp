...
namespace ngk { namespace chain { namespace config {
...
const static name system_account_name    { N(ngk) };
const static name null_account_name      { N(ngk.null) };
const static name producers_account_name { N(ngk.prods) };
...
const static name ngk_auth_scope       { N(ngk.auth) };
const static name ngk_all_scope        { N(ngk.all) };

const static name active_name     { N(active) };
const static name owner_name      { N(owner) };
const static name ngk_any_name  { N(ngk.any) };
const static name ngk_code_name { N(ngk.code) };
...
const static ngk::chain::wasm_interface::vm_type default_wasm_runtime = ngk::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace ngk::chain::config

constexpr uint64_t NGK_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / ngk::chain::config::percent_100;
}

template<typename Number>
Number NGK_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + ngk::chain::config::percent_100 - ngk::chain::config::percent_1)  / ngk::chain::config::percent_100;
}
