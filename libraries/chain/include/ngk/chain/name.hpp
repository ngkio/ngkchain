...
namespace fc {
  class variant;
  void to_variant(const ngk::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, ngk::chain::name& check);
} // fc
...
namespace ngk::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, ngk::chain::name& check);
...
#define N(X) ngk::chain::string_to_name(#X)

} // ngk::chain

namespace std {
   template<> struct hash<ngk::chain::name> : private hash<uint64_t> {
      typedef ngk::chain::name argument_type;
...
FC_REFLECT( ngk::chain::name, (value) )
