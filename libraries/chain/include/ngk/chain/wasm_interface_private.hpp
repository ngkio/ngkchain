...
#include <ngk/chain/webassembly/wavm.hpp>
#include <ngk/chain/webassembly/wabt.hpp>
#ifdef NGK_VM_OC_RUNTIME_ENABLED
#include <ngk/chain/webassembly/ngk-vm-oc.hpp>
#else
#define _REGISTER_NGKVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
#endif
...
using namespace fc;
using namespace ngk::chain::webassembly;
using namespace IR;
...
namespace ngk { namespace chain {
   ...
   namespace ngkvmoc { struct config; }
   ...
   struct wasm_interface_impl {
...
#ifdef NGK_VM_OC_RUNTIME_ENABLED
      struct ngkvmoc_tier {
         ngkvmoc_tier(const boost::filesystem::path& d, const ngkvmoc::config& c, const chainbase::database& db) : cc(d, c, db), exec(cc) {}
         ngkvmoc::code_cache_async cc;
         ngkvmoc::executor exec;
         ngkvmoc::memory mem;
      };
#endif

      wasm_interface_impl(wasm_interface::vm_type vm, bool ngkvmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const ngkvmoc::config& ngkvmoc_config) : db(d), wasm_runtime_time(vm) {
         if(vm == wasm_interface::vm_type::wabt)
            runtime_interface = std::make_unique<webassembly::wabt_runtime::wabt_runtime>();
#ifdef NGK_NGK_VM_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::ngk_vm)
            runtime_interface = std::make_unique<webassembly::ngk_vm_runtime::ngk_vm_runtime<ngk::vm::interpreter>>();
#endif
#ifdef NGK_NGK_VM_JIT_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::ngk_vm_jit)
            runtime_interface = std::make_unique<webassembly::ngk_vm_runtime::ngk_vm_runtime<ngk::vm::jit>>();
#endif
#ifdef NGK_VM_OC_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::ngk_vm_oc)
            runtime_interface = std::make_unique<webassembly::ngkvmoc::ngkvmoc_runtime>(data_dir, ngkvmoc_config, d);
#endif
         if(!runtime_interface)
            NGK_THROW(wasm_exception, "${r} wasm runtime not supported on this platform and/or configuration", ("r", vm));

#ifdef NGK_VM_OC_RUNTIME_ENABLED
         if(ngkvmoc_tierup) {
            NGK_ASSERT(vm != wasm_interface::vm_type::ngk_vm_oc, wasm_exception, "You can't use NGK VM OC as the base runtime when tier up is activated");
            ngkvmoc.emplace(data_dir, ngkvmoc_config, d);
         }
#endif
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            NGK_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            NGK_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#ifdef NGK_VM_OC_RUNTIME_ENABLED
         if(ngkvmoc) for(auto it = first_it; it != last_it; it++)
            ngkvmoc->cc.free_code(it->code_hash, it->vm_version);
#endif
         ...
         if(!it->module) {
            ...
            if (runtime_interface->inject_module(module)) {
               try {
                  ...
                  NGK_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
               } catch (const IR::ValidationException& e) {
                  NGK_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
...
#ifdef NGK_VM_OC_RUNTIME_ENABLED
      fc::optional<ngkvmoc_tier> ngkvmoc;
#endif
   };
...
#define _REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   ...
   _REGISTER_NGK_VM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)       \
   _REGISTER_NGKVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, NGK_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
