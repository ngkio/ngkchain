...
int main(int, char*[]) {
    ...
    OutputStream ngk(os, eis.GetType(), eis.HasBOM());                      // NEW
    PrettyWriter<OutputStream, UTF8<>, AutoUTF<unsigned> > writer(ngk);     // CHANGED
#else
    ...
    OutputStream ngk(os, true);                                             // NEW
    PrettyWriter<OutputStream, UTF8<>, UTF16LE<> > writer(ngk);             // CHANGED
#endif
