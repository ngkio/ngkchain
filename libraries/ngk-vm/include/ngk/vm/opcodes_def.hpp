...
/* clang-format off */
#define NGK_VM_CONTROL_FLOW_OPS(opcode_macro)   \
...
#define NGK_VM_BR_TABLE_OP(opcode_macro)        \
   opcode_macro(br_table, 0x0E)
#define NGK_VM_RETURN_OP(opcode_macro)          \
   opcode_macro(return_, 0x0F)
#define NGK_VM_CALL_OPS(opcode_macro)           \
   opcode_macro(call, 0x10)                     \
   opcode_macro(call_indirect, 0x11)            \
   opcode_macro(br_table_data, 0x12)
#define NGK_VM_CALL_IMM_OPS(opcode_macro)       \
...
#define NGK_VM_PARAMETRIC_OPS(opcode_macro)     \
...
#define NGK_VM_VARIABLE_ACCESS_OPS(opcode_macro)\
...
#define NGK_VM_MEMORY_OPS(opcode_macro)         \
...
#define NGK_VM_I32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i32_const, 0x41)
#define NGK_VM_I64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i64_const, 0x42)
#define NGK_VM_F32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f32_const, 0x43)
#define NGK_VM_F64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f64_const, 0x44)
#define NGK_VM_COMPARISON_OPS(opcode_macro)     \
...
#define NGK_VM_NUMERIC_OPS(opcode_macro)        \
...
#define NGK_VM_CONVERSION_OPS(opcode_macro)     \
...
#define NGK_VM_EXIT_OP(opcode_macro)            \
   opcode_macro(exit, 0xC0)
#define NGK_VM_EMPTY_OPS(opcode_macro)          \
...
#define NGK_VM_ERROR_OPS(opcode_macro)          \
   opcode_macro(error, 0xFF)

/* clang-format on */

#define NGK_VM_CREATE_ENUM(name, code) name = code,

#define NGK_VM_CREATE_STRINGS(name, code) #name,

#define NGK_VM_CREATE_MAP(name, code) { code, #name },

#define NGK_VM_OPCODE_NAME_if_
#define NGK_VM_OPCODE_NAME_else_
#define NGK_VM_OPCODE_NAME_return_

#define NGK_VM_OPCODE_NAME_TEST() 1
#define NGK_VM_OPCODE_NAME_TEST_NGK_VM_OPCODE_NAME_TEST 0,
#define NGK_VM_OPCODE_NAME_TEST_1 1, ignore
#define NGK_VM_EXPAND(x) x
#define NGK_VM_CAT2(x, y) x ## y
#define NGK_VM_CAT(x, y) NGK_VM_CAT2(x, y)
#define NGK_VM_APPLY(f, args) f args
#define NGK_VM_FIX_OPCODE_NAME_0(name) name ## _t
#define NGK_VM_FIX_OPCODE_NAME_1(name) name ## t
#define NGK_VM_FIX_OPCODE_NAME(iskeyword, garbage) NGK_VM_FIX_OPCODE_NAME_ ## iskeyword

#define NGK_VM_OPCODE_T(name)                                                             \
   NGK_VM_APPLY(NGK_VM_FIX_OPCODE_NAME,                                                   \
      (NGK_VM_CAT(NGK_VM_OPCODE_NAME_TEST_,                                               \
           NGK_VM_EXPAND(NGK_VM_OPCODE_NAME_TEST NGK_VM_OPCODE_NAME_ ## name ()))))(name)

#define NGK_VM_CREATE_EXIT_TYPE(name, code)                                                                      \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t pc;                                                                                                     \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define NGK_VM_CREATE_CONTROL_FLOW_TYPES(name, code)                                                                   \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() {}                                                                                       \
      NGK_VM_OPCODE_T(name)(uint32_t data) : data(data) {}                                                             \
      NGK_VM_OPCODE_T(name)(uint32_t d, uint32_t pc, uint16_t i, uint16_t oi)                                          \
...
#define NGK_VM_CREATE_BR_TABLE_TYPE(name, code)                                                                        \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
...
#define NGK_VM_CREATE_TYPES(name, code)                                                                                \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define NGK_VM_CREATE_CALL_TYPES(name, code)                                                                           \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
...
#define NGK_VM_CREATE_CALL_IMM_TYPES(name, code)                                                                       \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      uint16_t locals;                                                                                                 \
...
#define NGK_VM_CREATE_VARIABLE_ACCESS_TYPES(name, code)                                                                \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
...
#define NGK_VM_CREATE_MEMORY_TYPES(name, code)                                                                         \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
...
#define NGK_VM_CREATE_I32_CONSTANT_TYPE(name, code)                                                                    \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
      explicit NGK_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit NGK_VM_OPCODE_T(name)(int32_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         int32_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define NGK_VM_CREATE_I64_CONSTANT_TYPE(name, code)                                                                    \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
      explicit NGK_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit NGK_VM_OPCODE_T(name)(int64_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         int64_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define NGK_VM_CREATE_F32_CONSTANT_TYPE(name, code)                                                                    \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
      explicit NGK_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit NGK_VM_OPCODE_T(name)(float n) { data.f = n; }                                                          \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         float    f;                                                                                                   \
      } data;                                                                                                          \
...
#define NGK_VM_CREATE_F64_CONSTANT_TYPE(name, code)                                                                    \
   struct NGK_VM_OPCODE_T(name) {                                                                                      \
      NGK_VM_OPCODE_T(name)() = default;                                                                               \
      explicit NGK_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit NGK_VM_OPCODE_T(name)(double n) { data.f = n; }                                                         \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         double   f;                                                                                                   \
      } data;                                                                                                          \
...
#define NGK_VM_IDENTITY(name, code) ngk::vm::NGK_VM_OPCODE_T(name),
#define NGK_VM_IDENTITY_END(name, code) ngk::vm::NGK_VM_OPCODE_T(name)
