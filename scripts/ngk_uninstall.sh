...
function usage() {
  ...
  -i DIR      Directory where ngk is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the ngk data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export NGK_INSTALL_DIR=${INSTALL_LOCATION:-$NGK_INSTALL_DIR}

if [[ ! -d "${NGK_INSTALL_DIR}" ]]; then
   echo "[NGK installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${NGK_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/ngk/ngk/issues/6940
   echo "[NGK installation found: ${NGK_INSTALL_DIR}]" && INSTALL_PATHS+=("${NGK_INSTALL_DIR}") # NGK_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${NGK_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/ngk")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/ngk")
            fi
            # Version < 1.8.0; Before we started using ~/ngk/1.8.x
            ...
            echo " - NGK Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled NGK Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing NGK Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/ngk_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
