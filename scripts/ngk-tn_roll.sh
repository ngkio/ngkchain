#!/bin/bash
#
# ngk-tn_roll is used to have all of the instances of the NGK daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: ngk-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./ngk-tn_roll.sh is sufficient.
#

if [ -z "$NGK_HOME" ]; then
    echo NGK_HOME not set - $0 unable to proceed.
    exit -1
fi
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for ngkd or ngkd or nodengk
    exit 1
fi
...
if [ $DD = "all" ]; then
    for NGK_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $NGK_HOME/scripts/ngk-tn_up.sh "$*"
    done
else
    bash $NGK_HOME/scripts/ngk-tn_up.sh "$*"
fi
unset NGK_RESTART_DATA_DIR
