#!/bin/bash
#
# ngk-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# ngk-tn_bounce.sh and ngk-tn_roll.sh scripts.

connected="0"

rundir=programs/nodengk
prog=nodengk
...
datadir=var/lib/node_$NGK_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/ngk/node_$NGK_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/ngk/node_$NGK_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
